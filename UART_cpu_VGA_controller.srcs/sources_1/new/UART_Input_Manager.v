...
(
    ...
    BAUD_RATE = 480000,		       // Скорость передачи данных по UART (в бод)
	DIGIT_RANK = 6                 // Разрядность выходных данных
)
(
	...
	output reg [DIGIT_RANK - 1 : 0] out,      // Принятое декодированное DIGIT_RANK-разрядное число
	output reg ready_out,					  // Сигнал о том, что вышеупомянутое число сформировано
	output reg end_command
);

// UART_RX-соединители
wire UART_RX_Ready_Out;		  // Сигнал о том, что UART_RX был принят один пакет данных
wire [7:0] UART_RX_Data_Out;  // Принятый пакет данных находится на выходе UART_RX

localparam RX_DATA_SIZE = 8;

// Автомат, занимающийся приёмом данных по UART
...
// Дешифратор, преобразующий ASCII код с разрядностью RX_DATA_SIZE в DIGIT_RANK-разрядное число
